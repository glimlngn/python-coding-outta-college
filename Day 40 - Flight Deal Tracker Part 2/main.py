...
def get_cheapest_flights_list(sheets_data_df, month_start, month_end, origin_city, duration):
    ...
    for index, row in sheets_data_df.iterrows():
        ...
        print('Runtime:', round((end_time - start_time).total_seconds()), 'seconds')
        print()
        start_time = end_time
        cheapest_flights_list.append(cheapest_flight)
        
        # notif.show_toast("Flight Search Done", f"From {origin_city} to {destination_city}")
...
# if 'iataCode' not in sheets_data_df:
#     sheets_data_df['iataCode'] = None
