...
class DataManager:
    ...
    def get_sheets_data(self):
        response = requests.get(url=SHEETS_ENDPOINT+'/prices', headers=self.auth_header)
        data = response.json()
        return data['prices']

    def add_city_code_to_sheets(self, objectid, city_code):
        requests.put(url=f"{SHEETS_ENDPOINT+'/prices'}/{objectid+2}", # WRITE per row
    ...
    def get_email_list(self):
        response = requests.get(url=SHEETS_ENDPOINT+'/users', headers=self.auth_header)
        data = response.json()
        return data['users']